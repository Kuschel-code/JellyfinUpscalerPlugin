...
class SMBHandler:
    ...
    def get_full_path(self, share, path=""):
        # Clean host: remove leading backslashes and split if path included
        host = self.host.lstrip("\\").split("\\")[0]
        # Clean share
        share = share.strip("\\")
    ...
    def copy_file_between_shares(self, source_share, source_path, target_share, target_path):
        source_full = self.get_full_path(source_share, source_path)
        target_full = self.get_full_path(target_share, target_path)
        try:
            with open_file(source_full, mode="rb") as source_file:
                with open_file(target_full, mode="wb") as target_file:
