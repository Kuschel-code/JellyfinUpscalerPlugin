...
(function () {
  ...
  function currentFacts() {
    ...
    out += "- Importable community models: the 'Importable models' page (models-import.html) lists OpenModelDB models usable with the plugin (ready ONNX + convertible; refreshed weekly). Import directly on the plugin config page (card 'Import Community Model', v1.8.3.6+; whole catalog incl. .pth conversion with the docker7-converter image since v1.8.3.8) - or via the :5000 dashboard, or manually via POST /models/upload (validated, then listed in the plugin).\n";
    return out + "\n";
  }
  ...
  function buildCSS() {
    var css = ""
      + ".sb-fab{position:fixed;right:20px;bottom:20px;z-index:9998;width:56px;height:56px;border-radius:50%;border:0;cursor:pointer;background:#3b82f6;color:#ffffff;font-size:24px;box-shadow:0 6px 20px rgba(0,0,0,.35)}"
      + ".sb-fab:hover{filter:brightness(1.08)}"
      + ".sb-panel{position:fixed;right:20px;bottom:88px;z-index:9999;width:380px;height:520px;max-width:calc(100vw - 32px);max-height:calc(100vh - 120px);min-width:300px;min-height:340px;display:none;flex-direction:column;background:#11141b;color:#e6e9ef;border:1px solid #1f2430;border-radius:14px;box-shadow:0 12px 40px rgba(0,0,0,.5);overflow:hidden;font:14px/1.5 system-ui,-apple-system,Segoe UI,Roboto,sans-serif}"
      + ".sb-panel.open{display:flex}"
      + ".sb-grip{position:absolute;top:0;left:0;width:20px;height:20px;cursor:nwse-resize;z-index:3;background:linear-gradient(135deg,#3b82f6 0,#3b82f6 38%,transparent 38%);border-top-left-radius:14px;opacity:.55}"
      + ".sb-grip:hover{opacity:1}"
      + ".sb-head{display:flex;align-items:center;gap:8px;padding:12px 14px 12px 22px;background:#161a23;border-bottom:1px solid #1f2430}"
      + ".sb-head-t{display:flex;flex-direction:column;min-width:0}"
      + ".sb-head b{font-size:14px}.sb-head .sb-sub{color:#8b93a7;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}"
      + ".sb-lang{margin-left:auto;background:#0c0f16;color:#c7cdd9;border:1px solid #2a3040;border-radius:6px;font-size:11px;padding:3px 4px;cursor:pointer}"
      + ".sb-x{background:0;border:0;color:#8b93a7;font-size:20px;cursor:pointer;line-height:1}"
      ...
      + ".sb-bot{background:#1f2430;align-self:flex-start;border-bottom-left-radius:3px}"
      + ".sb-user{background:#3b82f6;color:#ffffff;align-self:flex-end;border-bottom-right-radius:3px}"
      + ".sb-msg code{background:#0c0f16;padding:1px 5px;border-radius:4px;font-size:12px}"
      + ".sb-msg pre{position:relative;background:#0c0f16;padding:9px;border-radius:6px;overflow-x:auto;margin:6px 0}.sb-msg pre code{background:0;padding:0}"
      + ".sb-msg a{color:#3b82f6}"
      + ".sb-copy{position:absolute;top:6px;right:6px;background:#3b82f6;color:#ffffff;border:0;border-radius:5px;font-size:10px;padding:2px 7px;cursor:pointer;opacity:.85}.sb-copy:hover{opacity:1}"
      + ".sb-note{margin-top:6px;font-size:11px;color:#8b93a7;font-style:italic}"
      ...
      + ".sb-rel{margin-top:8px;font-size:12px;color:#8b93a7}.sb-rel a{margin-right:2px}"
      + ".sb-chips{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}"
      + ".sb-chip,.sb-ghsearch{background:#0c0f16;border:1px solid #2a3040;color:#c7cdd9;border-radius:999px;padding:5px 10px;font-size:12px;cursor:pointer;text-align:left}"
      + ".sb-chip:hover,.sb-ghsearch:hover{border-color:#3b82f6;color:#fff}"
      + ".sb-foot{display:flex;gap:8px;padding:10px;border-top:1px solid #1f2430;background:#161a23}"
      + ".sb-foot input{flex:1;background:#0c0f16;border:1px solid #2a3040;color:#e6e9ef;border-radius:8px;padding:9px 10px;font-size:14px}"
      + ".sb-foot input:focus{outline:0;border-color:#3b82f6}"
      + ".sb-foot button{background:#3b82f6;color:#ffffff;border:0;border-radius:8px;padding:0 14px;font-weight:600;cursor:pointer}";
