...
const i18n = {
    en: {
        ...
        hero: {
            badge: "v1.5.2.1 — Security & Bug Fix",
        ...
        troubleshooting: {
            ...
            problems: [
                ...
                { title: "GPU Not Detected", desc: "Container runs in CPU mode despite GPU available.", solutions: ["NVIDIA: Install nvidia-container-toolkit, use --gpus all", "Intel: Use :1.5.3-intel tag with --device=/dev/dri", "AMD: Use :1.5.3-amd tag with --device=/dev/kfd --device=/dev/dri", "Windows: Use :1.5.3-cpu (GPU passthrough not supported in Docker Desktop)"], commands: [{ label: "Test NVIDIA", code: "docker run --rm --gpus all nvidia/cuda:12.2.2-base-ubuntu22.04 nvidia-smi" }, { label: "Test Intel", code: "docker exec jellyfin-ai-upscaler ls -la /dev/dri/" }] },
                { title: "Windows + Intel/AMD GPU", desc: "GPU images don't work on Windows Docker Desktop.", solutions: ["--device=/dev/dri is Linux-only (not available in WSL2)", "Use the CPU image: :1.5.3-cpu", "For GPU acceleration, run Docker on a Linux host", "CPU mode still produces excellent results, just slower"], commands: [{ label: "Windows CPU command", code: "docker run -d --name jellyfin-ai-upscaler -p 5000:5000 -p 2222:22 kuscheltier/jellyfin-ai-upscaler:1.5.3-cpu" }] },
        ...
        dockerTags: {
            ...
            cards: [
                { brand: "NVIDIA", tech: "CUDA + TensorRT", tag: ":1.5.3", models: "RTX 40/30/20, GTX 16/10", rating: 5, color: "#76b900" },
                { brand: "AMD", tech: "ROCm", tag: ":1.5.3-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" },
                { brand: "Intel", tech: "OpenVINO", tag: ":1.5.3-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" },
                { brand: "Apple", tech: "ARM64 Optimized", tag: ":1.5.3-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" },
                { brand: "CPU", tech: "Multi-Thread", tag: ":1.5.3-cpu", models: "Any x86 / ARM64", rating: 2, color: "#6366f1" }
            ]
        },
        changelog: {
            tag: "History",
    ...
    de: {
        ...
        hero: {
            badge: "v1.5.2.1 — Sicherheit & Bugfixes",
        ...
        troubleshooting: {
            ...
            problems: [
                ...
                { title: "GPU nicht erkannt", desc: "Container läuft im CPU-Modus trotz GPU.", solutions: ["NVIDIA: nvidia-container-toolkit installieren, --gpus all nutzen", "Intel: :1.5.3-intel Tag mit --device=/dev/dri nutzen", "AMD: :1.5.3-amd Tag mit --device=/dev/kfd --device=/dev/dri", "Windows: :1.5.3-cpu nutzen (GPU-Passthrough nicht in Docker Desktop)"], commands: [{ label: "NVIDIA testen", code: "docker run --rm --gpus all nvidia/cuda:12.2.2-base-ubuntu22.04 nvidia-smi" }, { label: "Intel testen", code: "docker exec jellyfin-ai-upscaler ls -la /dev/dri/" }] },
                { title: "Windows + Intel/AMD GPU", desc: "GPU-Images funktionieren nicht mit Docker Desktop.", solutions: ["--device=/dev/dri ist nur unter Linux verfügbar", "CPU-Image nutzen: :1.5.3-cpu", "Für GPU-Beschleunigung Docker auf Linux-Host verwenden", "CPU-Modus liefert trotzdem exzellente Ergebnisse"], commands: [{ label: "Windows CPU Befehl", code: "docker run -d --name jellyfin-ai-upscaler -p 5000:5000 -p 2222:22 kuscheltier/jellyfin-ai-upscaler:1.5.3-cpu" }] },
        ...
        dockerTags: {
            ...
            cards: [
                ...
                { brand: "AMD", tech: "ROCm", tag: ":1.5.3-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" },
                { brand: "Intel", tech: "OpenVINO", tag: ":1.5.3-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" },
                { brand: "Apple", tech: "ARM64 Optimiert", tag: ":1.5.3-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" },
                { brand: "CPU", tech: "Multi-Thread", tag: ":1.5.3-cpu", models: "Beliebig x86 / ARM64", rating: 2, color: "#6366f1" }
            ]
        },
        changelog: {
            tag: "Verlauf",
    ...
    fr: {
        ...
        hero: {
            badge: "v1.5.2.1 — Sécurité & Corrections",
        ...
        dockerTags: {
            ...
            cards: [
                ...
                { brand: "AMD", tech: "ROCm", tag: ":1.5.3-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" },
                { brand: "Intel", tech: "OpenVINO", tag: ":1.5.3-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" },
                { brand: "Apple", tech: "ARM64 Optimisé", tag: ":1.5.3-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" },
                { brand: "CPU", tech: "Multi-Thread", tag: ":1.5.3-cpu", models: "x86 / ARM64", rating: 2, color: "#6366f1" }
            ]
        },
        changelog: {
    ...
    zh: {
        ...
        hero: { badge: "v1.5.2.1 — 安全与修复", title1: "用人工智能", title2: "转换您的媒体。", subtitle: "使用神经网络将SD升级到4K。支持NVIDIA、AMD、Intel和Apple Silicon的GPU加速Docker微服务。", getStarted: "开始使用", viewGithub: "在GitHub上查看", stats: { gpus: "GPU架构", size: "插件大小", upscale: "升级", license: "开源" } },
        ...
        dockerTags: { tag: "Docker", title1: "选择你的", title2: "镜像。", cards: [{ brand: "NVIDIA", tech: "CUDA + TensorRT", tag: ":1.5.3", models: "RTX 40/30/20, GTX 16/10", rating: 5, color: "#76b900" }, { brand: "AMD", tech: "ROCm", tag: ":1.5.3-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" }, { brand: "Intel", tech: "OpenVINO", tag: ":1.5.3-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" }, { brand: "Apple", tech: "ARM64优化", tag: ":1.5.3-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" }, { brand: "CPU", tech: "多线程", tag: ":1.5.3-cpu", models: "任意x86/ARM64", rating: 2, color: "#6366f1" }] },
        changelog: { tag: "日志", title1: "最新", title2: "更新。", versions: [{ ver: "1.5.2.1", date: "2026年3月", type: "安全", items: ["安全：防止SSH命令注入（正则+ArgumentList）", "安全：媒体路径穿越防护", "安全：SSH加固——仅密钥认证", "修复：进度跟踪（卡在0%/50%）", "修复：暂停/恢复作业ID", "同步：插件与后端模型列表（14个模型）", "Docker 1.5.3：Scale验证、异步I/O、固定AMD基础镜像"] }, { ver: "1.5.2.0", date: "2026年2月", type: "修复", items: ["修复：GPU加速不工作（Issue #44）", "Docker基础镜像升级至cuDNN 9", "智能ONNX Provider检测", "NVIDIA GPU现在正确使用CUDA/TensorRT"] }, { ver: "1.5.1.1", date: "2026年2月", type: "修复", items: ["修复：SSH配置未保存", "新增：SSH连接测试按钮", "新增：API端点"] }, { ver: "1.5.0.0", date: "2026年1月", type: "重大", items: ["Docker微服务架构", "插件大小：417MB→1.6MB", "Web UI管理界面"] }] },
        sshSetup: {
    ...
    ru: {
        ...
        hero: { badge: "v1.5.2.1 — Безопасность и исправления", title1: "Преобразуйте медиа", title2: "с помощью ИИ.", subtitle: "Масштабируйте SD до 4K с помощью нейросетей. GPU-ускоренный Docker-микросервис для Jellyfin.", getStarted: "Начать", viewGithub: "GitHub", stats: { gpus: "Архитектур GPU", size: "Размер плагина", upscale: "Масштабирование", license: "Open Source" } },
    ...
    ja: {
        ...
        hero: { badge: "v1.5.2.1 — セキュリティとバグ修正", title1: "AIでメディアを", title2: "変換する。", subtitle: "ニューラルネットワークでSDを4Kにアップスケール。NVIDIA、AMD、Intel、Apple Silicon対応のGPU対応Dockerマイクロサービス。", getStarted: "始める", viewGithub: "GitHub", stats: { gpus: "GPUアーキテクチャ", size: "プラグインサイズ", upscale: "アップスケール", license: "オープンソース" } },
        ...
        dockerTags: { tag: "Docker", title1: "イメージを", title2: "選択。", cards: [{ brand: "NVIDIA", tech: "CUDA + TensorRT", tag: ":1.5.3", models: "RTX 40/30/20, GTX 16/10", rating: 5, color: "#76b900" }, { brand: "AMD", tech: "ROCm", tag: ":1.5.3-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" }, { brand: "Intel", tech: "OpenVINO", tag: ":1.5.3-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" }, { brand: "Apple", tech: "ARM64最適化", tag: ":1.5.3-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" }, { brand: "CPU", tech: "マルチスレッド", tag: ":1.5.3-cpu", models: "x86 / ARM64", rating: 2, color: "#6366f1" }] },
        changelog: { tag: "履歴", title1: "新機能", title2: "のご紹介。", versions: [{ ver: "1.5.2.1", date: "2026年3月", type: "セキュリティ", items: ["セキュリティ：SSHコマンドインジェクション防止（正規表現+ArgumentList）", "セキュリティ：パストラバーサル防止", "セキュリティ：SSH強化——鍵認証のみ、条件付きsshd起動", "修正：進捗追跡（0%/50%で停止）", "修正：一時停止/再開ジョブID解決", "同期：プラグインとバックエンド間のモデルリスト（14モデル）", "Docker 1.5.3：スケール検証、非同期I/O、AMDベースイメージ固定"] }, { ver: "1.5.2.0", date: "2026年2月", type: "修正", items: ["GPUアクセラレーション修正 (Issue #44)", "Docker基盤をcuDNN 9に更新", "ONNX Provider自動検出", "NVIDIA GPUがCUDA/TensorRTを正しく使用"] }, { ver: "1.5.1.1", date: "2026年2月", type: "修正", items: ["SSH設定の保存修正", "SSH接続テストボタン追加", "APIエンドポイント追加"] }, { ver: "1.5.0.0", date: "2026年1月", type: "メジャー", items: ["Dockerマイクロサービスアーキテクチャ", "プラグインサイズ削減", "Web UI"] }] },
        sshSetup: {
