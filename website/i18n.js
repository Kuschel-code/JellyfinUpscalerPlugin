...
const i18n = {
    en: {
        ...
        hero: {
            badge: "v1.5.2 — GPU Acceleration Fix",
        ...
        troubleshooting: {
            ...
            problems: [
                ...
                { title: "GPU Not Detected", desc: "Container runs in CPU mode despite GPU available.", solutions: ["NVIDIA: Install nvidia-container-toolkit, use --gpus all", "Intel: Use :1.5.2-intel tag with --device=/dev/dri", "AMD: Use :1.5.2-amd tag with --device=/dev/kfd --device=/dev/dri", "Windows: Use :1.5.2-cpu (GPU passthrough not supported in Docker Desktop)"], commands: [{ label: "Test NVIDIA", code: "docker run --rm --gpus all nvidia/cuda:12.2.2-base-ubuntu22.04 nvidia-smi" }, { label: "Test Intel", code: "docker exec jellyfin-ai-upscaler ls -la /dev/dri/" }] },
                { title: "Windows + Intel/AMD GPU", desc: "GPU images don't work on Windows Docker Desktop.", solutions: ["--device=/dev/dri is Linux-only (not available in WSL2)", "Use the CPU image: :1.5.2-cpu", "For GPU acceleration, run Docker on a Linux host", "CPU mode still produces excellent results, just slower"], commands: [{ label: "Windows CPU command", code: "docker run -d --name jellyfin-ai-upscaler -p 5000:5000 -p 2222:22 kuscheltier/jellyfin-ai-upscaler:1.5.2-cpu" }] },
        ...
        dockerTags: {
            ...
            cards: [
                { brand: "NVIDIA", tech: "CUDA + TensorRT", tag: ":1.5.2", models: "RTX 40/30/20, GTX 16/10", rating: 5, color: "#76b900" },
                { brand: "AMD", tech: "ROCm", tag: ":1.5.2-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" },
                { brand: "Intel", tech: "OpenVINO", tag: ":1.5.2-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" },
                { brand: "Apple", tech: "ARM64 Optimized", tag: ":1.5.2-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" },
                { brand: "CPU", tech: "Multi-Thread", tag: ":1.5.2-cpu", models: "Any x86 / ARM64", rating: 2, color: "#6366f1" }
            ]
        },
        changelog: {
            tag: "History",
            ...
            versions: [
                { ver: "1.5.2.1", date: "Feb 2026", type: "Fix", items: ["Fixed: Intel GPU not detected in Docker (Issue #45)", "Fixed: TensorRT crash — robust fallback to CUDA (Issue #46)", "Added: Intel GPU detection via /dev/dri, lspci, sysfs", "Added: Full TensorRT 10 runtime in NVIDIA image", "Improved: Automatic GPU provider chain (TensorRT → CUDA → CPU)"] },
    ...
    de: {
        ...
        hero: {
            badge: "v1.5.2 — GPU-Beschleunigung Fix",
        ...
        troubleshooting: {
            ...
            problems: [
                ...
                { title: "GPU nicht erkannt", desc: "Container läuft im CPU-Modus trotz GPU.", solutions: ["NVIDIA: nvidia-container-toolkit installieren, --gpus all nutzen", "Intel: :1.5.2-intel Tag mit --device=/dev/dri nutzen", "AMD: :1.5.2-amd Tag mit --device=/dev/kfd --device=/dev/dri", "Windows: :1.5.2-cpu nutzen (GPU-Passthrough nicht in Docker Desktop)"], commands: [{ label: "NVIDIA testen", code: "docker run --rm --gpus all nvidia/cuda:12.2.2-base-ubuntu22.04 nvidia-smi" }, { label: "Intel testen", code: "docker exec jellyfin-ai-upscaler ls -la /dev/dri/" }] },
                { title: "Windows + Intel/AMD GPU", desc: "GPU-Images funktionieren nicht mit Docker Desktop.", solutions: ["--device=/dev/dri ist nur unter Linux verfügbar", "CPU-Image nutzen: :1.5.2-cpu", "Für GPU-Beschleunigung Docker auf Linux-Host verwenden", "CPU-Modus liefert trotzdem exzellente Ergebnisse"], commands: [{ label: "Windows CPU Befehl", code: "docker run -d --name jellyfin-ai-upscaler -p 5000:5000 -p 2222:22 kuscheltier/jellyfin-ai-upscaler:1.5.2-cpu" }] },
        ...
        dockerTags: {
            ...
            cards: [
                ...
                { brand: "AMD", tech: "ROCm", tag: ":1.5.2-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" },
                { brand: "Intel", tech: "OpenVINO", tag: ":1.5.2-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" },
                { brand: "Apple", tech: "ARM64 Optimiert", tag: ":1.5.2-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" },
                { brand: "CPU", tech: "Multi-Thread", tag: ":1.5.2-cpu", models: "Beliebig x86 / ARM64", rating: 2, color: "#6366f1" }
            ]
        },
        changelog: {
            tag: "Verlauf",
            ...
            versions: [
                { ver: "1.5.2.1", date: "Feb 2026", type: "Fix", items: ["Behoben: Intel GPU nicht erkannt in Docker (Issue #45)", "Behoben: TensorRT Absturz — robuster Fallback zu CUDA (Issue #46)", "Hinzugefügt: Intel GPU-Erkennung via /dev/dri, lspci, sysfs", "Hinzugefügt: Vollständige TensorRT 10 Runtime", "Verbessert: Automatische GPU Provider-Kette (TensorRT → CUDA → CPU)"] },
    ...
    fr: {
        ...
        hero: {
            badge: "v1.5.2 — Correction accélération GPU",
        ...
        dockerTags: {
            ...
            cards: [
                ...
                { brand: "AMD", tech: "ROCm", tag: ":1.5.2-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" },
                { brand: "Intel", tech: "OpenVINO", tag: ":1.5.2-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" },
                { brand: "Apple", tech: "ARM64 Optimisé", tag: ":1.5.2-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" },
                { brand: "CPU", tech: "Multi-Thread", tag: ":1.5.2-cpu", models: "x86 / ARM64", rating: 2, color: "#6366f1" }
            ]
        },
        changelog: {
    ...
    zh: {
        ...
        hero: { badge: "v1.5.2 — GPU加速修复", title1: "用人工智能", title2: "转换您的媒体。", subtitle: "使用神经网络将SD升级到4K。支持NVIDIA、AMD、Intel和Apple Silicon的GPU加速Docker微服务。", getStarted: "开始使用", viewGithub: "在GitHub上查看", stats: { gpus: "GPU架构", size: "插件大小", upscale: "升级", license: "开源" } },
        ...
        dockerTags: { tag: "Docker", title1: "选择你的", title2: "镜像。", cards: [{ brand: "NVIDIA", tech: "CUDA + TensorRT", tag: ":1.5.2", models: "RTX 40/30/20, GTX 16/10", rating: 5, color: "#76b900" }, { brand: "AMD", tech: "ROCm", tag: ":1.5.2-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" }, { brand: "Intel", tech: "OpenVINO", tag: ":1.5.2-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" }, { brand: "Apple", tech: "ARM64优化", tag: ":1.5.2-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" }, { brand: "CPU", tech: "多线程", tag: ":1.5.2-cpu", models: "任意x86/ARM64", rating: 2, color: "#6366f1" }] },
        changelog: { tag: "日志", title1: "最新", title2: "更新。", versions: [{ ver: "1.5.2.0", date: "2026年2月", type: "修复", items: ["修复：GPU加速不工作（Issue #44）", "Docker基础镜像升级至cuDNN 9", "智能ONNX Provider检测", "NVIDIA GPU现在正确使用CUDA/TensorRT"] }, { ver: "1.5.1.1", date: "2026年2月", type: "修复", items: ["修复：SSH配置未保存", "新增：SSH连接测试按钮", "新增：API端点"] }, { ver: "1.5.0.0", date: "2026年1月", type: "重大", items: ["Docker微服务架构", "插件大小：417MB→1.6MB", "Web UI管理界面"] }] },
        sshSetup: {
    ...
    ru: {
        ...
        hero: { badge: "v1.5.2 — GPU Исправление", title1: "Преобразуйте медиа", title2: "с помощью ИИ.", subtitle: "Масштабируйте SD до 4K с помощью нейросетей. GPU-ускоренный Docker-микросервис для Jellyfin.", getStarted: "Начать", viewGithub: "GitHub", stats: { gpus: "Архитектур GPU", size: "Размер плагина", upscale: "Масштабирование", license: "Open Source" } },
        ...
        dockerTags: { tag: "Docker", title1: "Выберите", title2: "образ.", cards: [{ brand: "NVIDIA", tech: "CUDA + TensorRT", tag: ":1.5.2", models: "RTX 40/30/20, GTX 16/10", rating: 5, color: "#76b900" }, { brand: "AMD", tech: "ROCm", tag: ":1.5.2-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" }, { brand: "Intel", tech: "OpenVINO", tag: ":1.5.2-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" }, { brand: "Apple", tech: "ARM64", tag: ":1.5.2-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" }, { brand: "CPU", tech: "Multi-Thread", tag: ":1.5.2-cpu", models: "x86 / ARM64", rating: 2, color: "#6366f1" }] },
        changelog: { tag: "Журнал", title1: "Что", title2: "нового.", versions: [{ ver: "1.5.2.0", date: "Фев 2026", type: "Исправ.", items: ["GPU ускорение исправлено (Issue #44)", "Docker база обновлена до cuDNN 9", "Интеллектуальное определение ONNX провайдеров", "NVIDIA GPU теперь корректно используют CUDA/TensorRT"] }, { ver: "1.5.1.1", date: "Фев 2026", type: "Исправ.", items: ["SSH конфигурация исправлена", "Кнопка теста SSH", "API эндпоинт"] }, { ver: "1.5.0.0", date: "Янв 2026", type: "Мажорный", items: ["Docker микросервис", "1,6 МБ вместо 417 МБ", "Web UI"] }] },
        sshSetup: {
    ...
    ja: {
        ...
        hero: { badge: "v1.5.2 — GPUアクセラレーション修正", title1: "AIでメディアを", title2: "変換する。", subtitle: "ニューラルネットワークでSDを4Kにアップスケール。NVIDIA、AMD、Intel、Apple Silicon対応のGPU対応Dockerマイクロサービス。", getStarted: "始める", viewGithub: "GitHub", stats: { gpus: "GPUアーキテクチャ", size: "プラグインサイズ", upscale: "アップスケール", license: "オープンソース" } },
        ...
        dockerTags: { tag: "Docker", title1: "イメージを", title2: "選択。", cards: [{ brand: "NVIDIA", tech: "CUDA + TensorRT", tag: ":1.5.2", models: "RTX 40/30/20, GTX 16/10", rating: 5, color: "#76b900" }, { brand: "AMD", tech: "ROCm", tag: ":1.5.2-amd", models: "RX 7000, RX 6000", rating: 4, color: "#ed1c24" }, { brand: "Intel", tech: "OpenVINO", tag: ":1.5.2-intel", models: "Arc A-Series, Iris Xe", rating: 4, color: "#0071c5" }, { brand: "Apple", tech: "ARM64最適化", tag: ":1.5.2-apple", models: "M1, M2, M3, M4", rating: 3, color: "#a2aaad" }, { brand: "CPU", tech: "マルチスレッド", tag: ":1.5.2-cpu", models: "x86 / ARM64", rating: 2, color: "#6366f1" }] },
        changelog: { tag: "履歴", title1: "新機能", title2: "のご紹介。", versions: [{ ver: "1.5.2.0", date: "2026年2月", type: "修正", items: ["GPUアクセラレーション修正 (Issue #44)", "Docker基盤をcuDNN 9に更新", "ONNX Provider自動検出", "NVIDIA GPUがCUDA/TensorRTを正しく使用"] }, { ver: "1.5.1.1", date: "2026年2月", type: "修正", items: ["SSH設定の保存修正", "SSH接続テストボタン追加", "APIエンドポイント追加"] }, { ver: "1.5.0.0", date: "2026年1月", type: "メジャー", items: ["Dockerマイクロサービスアーキテクチャ", "プラグインサイズ削減", "Web UI"] }] },
        sshSetup: {
